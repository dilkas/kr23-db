...
value_function_converged(_,_,_,_,[]) :-
    !.
value_function_converged(OldVF,NewVF,Dom,Epsilon,[Int | Ints]) :-
    gfodd_evaluation_by_ve(OldVF, Int, Dom, OldMap),
    gfodd_evaluation_by_ve(NewVF, Int, Dom, NewMap),
    Diff = OldMap - NewMap,
    !,
    abs(Diff) < Epsilon,
    !,
    value_function_converged(OldVF,NewVF,Dom,Epsilon,Ints).



value_function_converged(OldVF, NewVF) :-
...
max_over_actions([], VF1, Rewd, VF) :-
    ...
    reduce(VF2, VF),	
    !.
max_over_actions([-1 | ActionList], SoFar, Rewd, VF) :-
    max_over_actions(ActionList, SoFar, Rewd, VF),
    !.
max_over_actions([First | ActionList], SoFar, Rewd, VF) :-
    ...
    max_over_actions(ActionList, NewVFsofar, Rewd, VF).
...
regress(Round, VF, _, ActionList, ActionRules, [], RegVF) :-
	...
	VF = [VFRoot | _],
	  ...
	  ; 
       ...
       !,  
       max_over_actions(ActionList2, First, Rewd1, RegVF)),
...
regress(Round, VF, VFvars, Actionsofar, ActionRulesSoFar, [Op | OpList], RegVF) :- 
    ...
    (absorbingstate ->	
	apply(DiscRoot, [DiscRoot | DiscountedVF], RewdRoot, Rewd, max, FinalVF)
	;
        stdApart(Rewd, [RewdRoot1 | Rfn]),
        apply(DiscRoot, [DiscRoot | DiscountedVF], RewdRoot1, [RewdRoot | Rfn], +, FinalVF)),
    (debug(main1) -> (!, tryeval(FinalVF, Round, Op, 'FinalVF')) ; (true)),	
    (debug(main) -> (write('Added Rewd Function:'), nl, pp(FinalVF)) ; true),	
     FinalAction = Action,
     FinalActionParams = ActParams
     ;
    ...
    (once((Round == 1)),
     FinalVF = [node(FinalRoot, _, _, _) | _],     
     getDVars(FinalVF, FinalVars),
     getSubsKey(FinalActionParams, ActParamsKey),
%     getSubsKey(FinalVars, FinalVarsKey),
%     conc(ActParamsKey, FinalVarsKey, ActVarsKey),
%     conc(ActVarsKey, GoalKey, Key),
     variablizeTerm(FinalAction, ActParamsKey, ActionVar),
     variablizeDD(FinalVF, ActParamsKey, FinalVarVF),
     outpolicyfile3579(OPF),
     open(OPF, append, S), 
     tell(S),
    ...
    !,
    regress(Round, VF, VFvars, [ReducedVF | Actionsofar], ActionRulesSoFar1, OpList, RegVF).
