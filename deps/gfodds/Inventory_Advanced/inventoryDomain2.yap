...
templates([level0(_x1), not_level0(_x2), level1(_x13), not_level1(_x14), tfull(_x3), not_tfull(_x4), tin(_x5, _x6), not_tin(_x7, _x8), shop(_x9), not_shop(_x10), depot(_x11), not_depot(_x12), freq(_x15), not_freq(_x16), eq(_x45, _x46), not_eq(_x47, _x48)]).

arg_types([pred(level0, [location]), pred(level1, [location]), pred(tfull, [truck]), pred(tin, [truck, location]), pred(shop, [location]), pred(depot, [location]), pred(freq, [location])]).

preds([pred(level0, 1), pred(level1, 1), pred(tfull, 1), pred(tin, 2), pred(shop, 1), pred(depot, 1), pred(freq, 1), pred(eq, 2)]).

predOrder([eq, level0, level1, tfull, tin, shop, depot, freq]).
...
op(unload, [truck, location], class(unload(T2,P2), 0.0, preconds([tfull(T2), shop(P2), tin(T2,P2)]), effects([
cond([level0(P2)], [outcome(1.0, addList([not_level0(P2), level1(P2), not_tfull(T2)]), delList([tfull(T2), not_level1(P2), level0(P2)]))]),
cond([level1(P2)], [outcome(1.0, addList([not_level1(P2), not_tfull(T2)]), delList([tfull(T2), level1(P2)]))]),
cond([not_level0(P2),not_level1(P2)], [outcome(1.0, addList([]), delList([]))])
]))),
...
exoOps([
op(consume, [location], class(consume(P5), 0.0, preconds([shop(P5)]), effects(
[
cond([freq(P5),level1(P5)], [outcome(0.6, addList([level0(P5),not_level1(P5)]), delList([not_level0(P5),level1(P5)])), outcome(0.4, addList([]), delList([]))]),
cond([freq(P5),not_level0(P5),not_level1(P5)], [outcome(0.6, addList([level1(P5)]), delList([not_level1(P5)])), outcome(0.4, addList([]), delList([]))]),
...
operator(unload, [xh, xp], [node(0, 0.0, -1, -1)], [ev(succ, [node(1, 1.0, -1, -1)])]).
...
tvd(unload, succ, level0, [y2], [1], 6, [node(6,level0(y2),5,0), node(5,tfull(xh),4,1), node(4,tin(xh,y2),3,1), node(3,shop(y2),2,1),node(2,eq(y2,xp),0,1),node(1,1.0,-1,-1),node(0,0.0,-1,-1)]).

tvd(unload, succ, level1, [y3], [1], 7, [node(7,eq(y3,xp),6,2), node(6,tin(xh,y3),5,2), node(5,tfull(xh),4,2), node(4,shop(y3),3,2), node(3,level0(y3),1,0), node(2,level1(y3),1,0), node(1,1.0,-1,-1), node(0,0.0,-1,-1)]).
