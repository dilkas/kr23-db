...
fodd_inference_by_variable_elimination_mdd(I, [node(Index, Lit, L, R) | D]) :- 
    ...
    atom_concat([litM,- | IndexChars],LitM),
    functor(Lit,Pred,_),
    member([Pred | LitList], I),
    make_mdd_for_pred(Lit, LitList, LitM),

    atom_concat([neglitM,- | IndexChars], NegLitM),
    recorda(unitmdd, node(1.0,[]), UnitMDDRef),
    recorda(unitmdd, root(UnitMDDRef), _),
    mdd_apply(unitmdd,LitM, NegLitM, -),
    eraseall(unitmdd),

    (debug(ve) -> (write('Node Index: '), write(Index), nl) ; true),
    (debug(ve) -> (write('Table for true branch of literal '), write(Lit), write(' (LitTable)'), nl) ; true), 
    (debug(ve) -> (printmdd(LitM)) ; true),
    (debug(ve) -> (write('Table for false branch of literal '), write(Lit), write(' (NegLitTable)'), nl) ; true), 
    (debug(ve) -> (printmdd(NegLitM)) ; true),

    recorda(zeromdd, node(0.0,[Index-t]), ZeroMDDLRef),
    recorda(zeromdd, root(ZeroMDDLRef), _),
    number_chars(L,LChars), 
    atom_concat([nodeMDD,- | LChars], LM),
    atom_concat([LM, 'edgeadded'], LMedgeadded),    
    mdd_apply(LM, zeromdd, LMedgeadded, +),
    eraseall(zeromdd),    

    recorda(zeromdd, node(0.0,[Index-f]), ZeroMDDRRef),
    recorda(zeromdd, root(ZeroMDDRRef), _),
    number_chars(R,RChars), 
    atom_concat([nodeMDD,- | RChars], RM),
    atom_concat([RM, 'edgeadded'], RMedgeadded),    
    mdd_apply(RM, zeromdd, RMedgeadded, +),
    eraseall(zeromdd),

    (debug(ve) -> (write('Table bubbled up from left side of literal (LTable)  '), nl) ; true), 
    (debug(ve) -> (printmdd(LMedgeadded)) ; true),
    (debug(ve) -> (write('Table bubbled up from right side of literal (RTable)  '), nl) ; true),
    (debug(ve) -> (printmdd(RMedgeadded)) ; true),

    atom_concat([llitM,- | IndexChars],LLitM),
    mdd_apply(LitM, LMedgeadded, LLitM,*),
    eraseall(LMedgeadded),
    atom_concat([rlitM,- | IndexChars],RLitM),
    mdd_apply(NegLitM, RMedgeadded, RLitM, *),
    eraseall(RMedgeadded),     

    (debug(ve) -> (write('Multiplied LitTable and LTable (LLitTable)  '),nl) ; true), 
    (debug(ve) -> (printmdd(LLitM)) ; true),     
    (debug(ve) -> (write('Multiplied NegLitTable and RTable (RLitTable)  '),nl) ; true),
    (debug(ve) -> (printmdd(RLitM)) ; true),    

    atom_concat([nodeMDDunopt,- | IndexChars], NodeMunopt),
    mdd_apply(LLitM, RLitM, NodeMunopt, +),
    eraseall(LLitM),
    eraseall(RLitM),    

    (debug(ve) -> (write('NodeTable  (merged LLitTable and RLitTable)'), nl) ; true),
    (debug(ve) -> (printmdd(NodeMunopt)) ; true),
    
%    mdd_optimize(Index, NodeM, OptimizedM), 
    atom_concat([nodeMDD,- | IndexChars], NodeM),
(Index = 5, trace ; true),
    mdd_optimize_recursive(Index, NodeMunopt, NodeM),

    (debug(ve) -> (write('Optimized Node Table  '), nl) ; true),
    (debug(ve) -> (printmdd(NodeM)) ; true),
    (debug(ve) -> (write(' ------------ '), nl) ; true),
    !,
    fodd_inference_by_variable_elimination_mdd(I, D).
...
perform_gfodd_reduction_by_ve([I | Ints], D, Drev, DVars, EdgesSoFar, Edges, [Map | AllMaps]) :- 
	...
	(fodd_inference_by_variable_elimination_mdd(I, Drev),
	 D = [node(RootIndex,_,_,_) | _],
	 number_chars(RootIndex, RootIndexChars),
	 atom_concat([nodeMDD,- | RootIndexChars], RootNodeM),
	 write('Final MDD should be a leaf'),nl, 
	 printmdd(RootNodeM),
         recorded(RootNodeM, node(Map,MaxBlocks1), _),
%	 eraseall(RootNodeM),
	 MaxBlocks = [MaxBlocks1],
	 findall(_, (member(node(Index,_,_,_), D), once((number_chars(Index, IndexChars), atom_concat([nodeMDD,- | IndexChars], NodeM), eraseall(NodeM)))), _))
	;
