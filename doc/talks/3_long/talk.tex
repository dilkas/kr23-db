...
\definecolor{color3}{HTML}{e7298a} % TODO: I don't like pink
...
\begin{frame}{(Unweighted) First-Order Model Counting}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item Example formula: $\forall x \in \Delta\text{. } \pP(x) \lor \qQ(x)$
        \item Let $\Delta \coloneqq \{\, 1, 2 \,\}$
        \item Interpretations: all subsets of $\{\, \pP(1), \qQ(1), \pP(2), \qQ(2) \,\}$
        \item Models:
        \[
          \begin{matrix}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{overlayarea}{\textwidth}{4cm}
        \only<1>{
          \begin{venndiagram2sets}[labelNotAB={$\Delta$},labelA={$\pP$},labelB={$\qQ$}]
  ...
  \end{columns}
\end{frame}
% TODO: mention that n-ary predicates ~= relations
