...
\begin{frame}{Back to Our Example}
  ...
  \begin{itemize}
    ...
    \item But \alert{$\Theta(mn)$} is still much better than translating to
          ...
          \pause
    \item The rest of this talk is about how to find such functions
          automatically.
  \end{itemize}
\end{frame}
...
\begin{frame}{Resulting Improvements to Counting Functions}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item by hand: \alert{$\Theta(m)$}\pause
    \end{itemize}
    \item partial injections in \alert{$\Theta(mn)$} time
    \begin{itemize}
      \item by hand: \alert{$\Theta({\min\{\, m, n \,\}}^2)$}\pause
    \end{itemize}
    \item bijections in \alert{$\Theta(m)$} time
    \begin{itemize}
      \item \alert{optimal!}\pause
      \item experimentally, FastWFOMC runtime appears to be \alert{$\Omega(m^{4})$}
    \end{itemize}
  \end{itemize}
  \begin{tikzpicture}[remember picture,overlay]
