...
% TODO: make the talk longer, maybe elaborating on the function construction and
% a more careful explanation of the problem
...
\date{DTAI Seminar, 26th May 2023}
...
\begin{frame}{More Formally: What Is the Input?}
  \[
    \forall x \in \alert<2>{\Gamma}\text{. }\forall y, z \in \alert<2>{\Delta}\text{. }\pP(x, y) \land \pP(x, z) \Rightarrow \alert<4>{y=z}
  \]
  \begin{block}{\alert<2>{Many-Sorted} \alert<3>{Function-Free} First-Order Logic with \alert<4>{Equality}}
  \begin{itemize}
    \item<5-> Any number of variables
    \item<5-> All variables are bound
    \item<6-> \alert{$\forall$} and \alert{$\exists$} quantifiers can be nested arbitrarily deep
...
\begin{frame}{Exact Algorithms for FOMC}
  ...
  \end{block}
\end{frame}
% NOTE: explain what knowledge compilation is
...
\begin{frame}{How These Rules Fit Together (2/5)}
  ...
  \end{tikzpicture}
\end{frame}
% NOTE: need to explain how atom counting splits the domain
...
\begin{frame}{Resulting Improvements to Counting Functions}
  \Wider[3em]{ Let \alert{$\Gamma$} and \alert{$\Delta$} be two sets with
  ...
  \begin{tikzpicture}[remember picture,overlay]
    \node[draw,rectangle,color=gray] at ([xshift=-3.3cm,yshift=-0.2cm]current page.east) {\includegraphics{plot}};
  \end{tikzpicture}
\end{frame}
