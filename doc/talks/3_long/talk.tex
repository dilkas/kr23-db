...
\definecolor{highlightcolor}{HTML}{22373A}
...
% TODO: replace capital latin letters with capital Greek letters
...
\begin{frame}{What Computers Can and Cannot Do}
  ...
  \end{tikzpicture}
\end{frame}
% TODO: replace cat with 'Generate a picture of a monkey with a funny hat' (make sure it's legal to use)

\begin{frame}[fragile]{There Are Many Ways to Count}
  \begin{center}
    \begin{tikzcd}[remember picture]
      |[alias=WMI]|\WMI{} & \WFOMI{} \\
      |[alias=WMC]|\#\SAT{}/\WMC{} \ar[u] \ar[d] \ar[r] & \WFOMC{} \ar[u] \ar[d] \\
  ...
  \begin{tikzpicture}[overlay,remember picture]
    \begin{scope}[transform canvas={xshift=-3em}]
      \draw[->,draw=gray] ($(WMC)+(0,0.1)$)--($(WMI)+(0,0.5)$) node[midway,left] {\color{gray}{$\sum \to \int$}};
      \draw[->,draw=gray] ($(WMC)-(0,0.1)$)--($(PBP)-(0,0.5)$) node[midway,left] {\color{gray}{Generalising weights}};
    \end{scope}
    \begin{scope}[transform canvas={yshift=-2em}]
      \draw[->,draw=gray] ($(PBP)-(1,0)$)--($(SP)+(1,0)$) node[midway,below] {\color{gray}{From propositional to first-order logic}};
    \end{scope}
  \end{tikzpicture}
\end{frame}
% maybe mention #CSP and other non-SAT ways of counting (SMT?, counting paths in automata and directed graphs)
% TODO: gradually reveal parts of this diagram (1. axes + #SAT + WMC, 2. extension to FOL (with bottom label), 3. integration 4. bottom row)
% TODO: at the same time, show the full name and the reference for each at the bottom
% TODO: for future work, come back to this slide and add more arrows (e.g., beyond FOL)

% TODO: a big chunk of the introduction should provide motivation to my work (e.g., mention applications)
...
% TODO: maybe meat makes more sense than sandwich? Or a picture of recursion, e.g., fractals?
{ % all template changes are local to this group.
    \setbeamertemplate{navigation symbols}{}
    \begin{frame}<article:0>[plain]
        \begin{tikzpicture}[remember picture,overlay]
            \node[at=(current page.center)] {
                \includegraphics[keepaspectratio, width=\paperwidth]{sandwich.jpg}
            };
        \end{tikzpicture}
     \end{frame}
}
