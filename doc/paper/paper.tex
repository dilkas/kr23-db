...
\begin{itemize}
  \item We call a domain name \emph{atomic} if it is free of subtraction (e.g.,
        $m$, $n$); otherwise it is \emph{non-atomic} (e.g., $m-1$, $n-l$).
  \item We assume a fixed total ordering of all domains. In particular, in our
        running example we assume that $\Gamma$ goes before $\Delta$.
  ...
  \item $\mathsf{F}$ is a partial map from the set of FCG nodes to function
        names (e.g., $f$, $g$).
  \item Functions \newDomainName and \newFunctionName both generate
        previously-unused names. The latter also records the new name in
        $\mathsf{F}$.
...
interpretation of its only direct successor. The interpretation of a $\Reff$
node is a function call. The direct successor of the $\Reff$ node (say, $v$)
then must introduce a function. The parameters of this function are the sizes of
...
\begin{figure}
  ...
  \caption{Function calls and their return values when \cref{alg:interpretation}
    is run on the FCG from \cref{fig:examplefcg}.}\label{fig:functioncalls} % TODO: do I need a dot?
\end{figure}

% TODO: examine the order in which everything is introduced
\begin{example}\label{example:interpretation}
  ...
  $\begin{tikzcd} \langle\Gamma, \Delta\rangle \ar[r, mapsto, "\rho"] & \langle\Gamma', \Delta^{\bot}\rangle \ar[r, mapsto, "\Dthree"] & \langle m-1, n-l \rangle\end{tikzcd}$,
