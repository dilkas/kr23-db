...
\begin{algorithm}[t]
  ...
  $\variableNames \gets \{\, \Omega \mapsto \newVariableName{} \mid \Omega \in \mathcal{D} \,\}$\;
  ...
  \Fn{\visit{node $v$, variable names \variableNames}}{
    ...
    \For{$\Omega \in \mathfrak{D}(v)$ s.t. $\newVariableNames(\Omega)$ is non-atomic}{
  ...
  \Fn{\actuallyVisit{node $v$, variable names \variableNames}}{
    \Switch{label of $v$}{
      ...
      \uCase{$\CR_{\Omega \mapsto \Omega'}(v')$}{
        \Return{\visit{$v'$, $\variableNames \cup \{\, \Omega' \mapsto (\variableNames(\Omega) - 1) \,\}$}}\;
      }
      \uCase{$\Reff_{\rho}(v')$}{
        $\args \gets \langle \variableNames{$\rho(\Omega)$} \mid \Omega \in \mathfrak{D}(v') \rangle$\;
...
% \begin{itemize}
%   \item atomic --- no subtraction, i.e., just a variable name
%   \item for each FCG node $v$, $\mathfrak{D}(v)$ is a precomputed set of domains
%         that are used by $v$ and all of its successors (except the ones that are
%         introduced by $v$ or one of its successors) (this was already part of
%         \textsc{ForcLift}, although had to be reimplemented in a way that avoids
%         infinite loops when dealing with cycles in graphs)
%   \item for list generators over a set of domains, we assume an arbitrary fixed
%         ordering over all domains
%   \item $\mathsf{F}$ is a partial map from the set of FCG nodes to Strings,
%         i.e., function names
...
%   \item the top function is the main one
%   \item `is a direct successor of a $\Reff$ node' --- the in-degree is greater
%         than 1
% \end{itemize}
