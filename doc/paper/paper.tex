...
replaced one of the $\star$'s in $G$ according to a set order. We say that an
...
% TODO: will need to explain: hashing, caching, how the output of genMaps is filtered
  ...
  \Fn{\identifyRecursion{$\phi$, $\psi$, $\rho = \emptyset$}}{
    ...
    \ForEach{clause $c \in \psi$\label{line:for1}}{
      \ForEach{clause $d \in \phi$ s.t. $\#d=\#c$\label{line:for2}}{
        \ForAll{$(\beta, \gamma) \in \generateMaps{$c$, $d$, $\rho$}$\label{line:generateMaps}}{
...
function \texttt{r} is called to check whether a recursive call
...
$\rho(\Omega) \in \Doms(\phi)$. Otherwise, \texttt{r} returns
...
Function \texttt{r} iterates over pairs of clauses of $\phi$ and
$\psi$ that have the same hash codes. It uses function \texttt{genMaps} to
...
Given two clauses $c \in \psi$ and $d \in \phi$, \texttt{genMaps} considers
...
keyword in \texttt{genMaps} works as in programming languages such as C\#,
...
%   case, function \texttt{genMaps} would have $3!=6$ bijections to check.
...
%   \texttt{genMaps} is called with $c = c_2$, $d = d_2$, and
...
%   \texttt{r($\emptyset$, $\emptyset$, $\rho$)}, which
%   immediately returns
