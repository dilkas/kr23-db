...
% TODO: FOMC algorithms -> of most algorithms to date
\begin{abstract}
  ...
  allow us to find efficient solutions to counting fundamental structures such
  as injections and bijections.
\end{abstract}
% TODO: We conclude after report on empirical results and discuss a few
% conjectures on what correctness might look like.

\section{Introduction}

% TODO: It might be worth remarking either in the abstract or in the intro that
% generally the weighted version is what is studied but the nonweighted version
% is also still interesting because of its connections to finite model theory.
% You could make a reference to the work of Paul Beam on symmetric weighted
% model counting.

% 1. What is the problem?
% 2. Why is it interesting and important?

% in fact, the WFOMC algorithm by \citet{DBLP:conf/ijcai/BroeckTMDR11} can do so when it is unable to solve the instance in a lifted manner.
...
% TODO (footnote): You need to explain this a little bit more. You say that it
% is liftable in principle but you should clarif ythat factorial has never been
% shown empirically and there is no correctness theorem claiming that a
% factorial can be computed in a lifted manner. Or something like that.
...
% TODO: Because this is the focus of your work you might want to give at least
% one example of such a stringent condition
...
stages: compilation and evaluation/propagation.\footnote{There is also an
  ...
  of this paper.} In the first stage, various \emph{(compilation) rules} are
...
the circuit, computing the weighted model count. Along with new compilation
rules, \textsc{Crane} introduces changes to both stages of the process. First,
while \textsc{ForcLift} applies compilation rules via greedy\footnote{The
  algorithm is not described in any paper on \textsc{ForcLift} but can be found
  in its source code at \url{https://dtai.cs.kuleuven.be/drupal/wfomc}} search,
\textsc{Crane} uses a hybrid search algorithm that applies some rules greedily
and some using breadth-first search. Second, the product of compilation is not
directly evaluated but rather interpreted as a collection of functions on domain
sizes.
...
resulting formula is just like the input formula $\phi$ except it refers to a
...
instance. During compilation, new domains are added to $\mathcal{D}$ by some of
the compilation rules.
% TODO ('Let D... all domains'): You need to state explicitly or implicitly that
% to set of domains is constructed from a finite set of constants. Either you
% can define the set of constants in the langoage or you can say that in the
% interest of simplicity the set of constants is a set of nullary functions
% mentioned in the sentence, which only mentions relations or nullary functions.
% Function that take arguments are not allowed.
...
% TODO: You should mention that somewhat unusually, first order variables are in
% capital letters, similar to the convention of logic programming. They are not
% to be confused with second order qualification.
...
single source, node labels, and ordered outgoing edges.\footnote{Imposing an
  ordering on outgoing edges is just a limited version of edge labelling.} Node
...
% TODO: explain notation (i.e., remove the bot)
\begin{example}
...
The compatibility of a pair of clauses is checked by function \texttt{genMaps}.
It considers every possible bijection $\beta\colon \Vars(c) \to \Vars(d)$ and
map $\gamma\colon \Doms(c) \to \Doms(d)$ s.t.
\[
  \begin{tikzcd}
...
\end{table*}
% TODO: This is very neat but readers from the community will expect to see some
% plots of time. Can you also give a few of those that reflect or mirror the
% conclusions from the table? Also it is a standard to discuss a few lifted
% theories like the smokers and friends example. Can you also include some
% empirical results with plots of time on such theories as well to appeal to the
% sanity of the reader that it actually works for problems in frameworks such
% as Markov log networks
...
% TODO: You previously mentioned this class being discussed in another paper.
% You will need to remind the reader what you are doing differently to that and
% how you are improving to previous work.
...
constructing FCGs that indeed compute the right (weighted) model counts. Second,
these definitions must be simplified before they can be used, perhaps by a
computer algebra system. Third, most importantly, we need a way to find the base
cases for the recursive definitions provided by \textsc{Crane}. Fourth, since
the first solution found by \textsc{Crane} is not always optimal in terms of its
