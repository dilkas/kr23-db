...
% TODO: maybe something other than MC? Introduce the notation and the role of \sigma.
% TODO: add an argument about why this recursion is (or can easily be made) finite
\begin{theorem}[Correctness of $\Reff$]
\end{theorem}
\begin{proof} % TODO: finish (Proof technique: show a bijection between the sets of models)
  For a given formula $\phi$, \cref{alg:trycache} attempts to find a formula
  $\psi$ and a map $\rho\colon \Doms(\psi) \to \Doms(\phi)$ such that
  $\MC(\psi, \sigma \circ \rho\colon \Doms(\psi) \to \mathbb{N}_{0}) = \MC(\phi, \sigma\colon \Doms(\phi) \to \mathbb{N}_{0})$.
\end{proof}
...
\end{theorem}
\begin{proof}% TODO (I have some handwritten notes)
  % prove that all functions output by Crane can be computed in polytime (must, should be easy: 1) bound the number of possible arguments that any function can have, 2) prove that the computation time of any dynamic programming cell is always polytime (what about functions depending on other functions?))
\end{proof}

\section{Empirical Results}\label{sec:results}
% TODO: Add that the linear order axiom is also liftable. Cite the same paper in the
...
% TODO: also mention that most of the work for this paper was done at UoE
