...
% TODO: I found the choice for what goes into a footnote and what doesn't rather
% inconsistent, which made it a bit irritating to read for me. Maybe review if
% the current use of footnotes is really necessary and helpful?
% TODO: replace references with the condensed version
...
% TODO: also cite 'WFOMC with DAG axioms'
% \cite{DBLP:journals/corr/abs-2302-09830}
...
% TODO: too many acronyms (e.g., FOL)
...
% TODO (about the sentence "Lifted ... (Kersting)"): This developed
% simultaneously, with the development of so-called probabilistic relational
% models, that combined syntactic power of FOL with, b networks & m networks,
% allowing for a more relational view of uncertainty modelling
...
% TODO: must remove this footnote. Also, two more papers to mention:
% 1. the L2C paper has the factorial example as well. But then why doesn't L2C work on it?
% 2. cite the new paper "On Discovering Interesting Combinatorial Integer Sequences" as another example of computing n! using publicly unavailable code and as an example of applications to combinatorics
...
% TODO: I find the writing mostly clear, but the structure of the Preliminaries
% somewhat confusing. It starts with some notation, then some definitions, then
% suddenly a paragraph titled "Notation", and so on. Maybe see if you can
% rearrange some things and/or add some (more) helpful paragraph titles?
...
\end{definition}
% TODO: what is its semantic meaning? Meaning what logically: a \ne b? (say
% that)

% TODO: c = (L, E, \delta), E - inequality constraints
...
% TODO: The first paragraph already introduced some notation.
% TODO: so S^2 \coloneqq {(a, b) \mid a, b \in S}
\paragraph{Notation.}
...
$[n] \coloneqq \{\, 1, \dots, n \,\}$, e.g., $[0] = \emptyset$, and
...
% TODO: why was 'c-clause' a footnote but not this? Combine with previous
% footnote.
...
needed.
% TODO: explain why we're not losing anything by dropping equality constraints
% (e.g., X = Y). Answer: having two variables that are equal is the same thing
% as having one variable (since by the definition of a clause, this equality can
% only be at the beginning of a clause.

% TODO: the 'but note' part is confusing
% TODO: add \, for when a line ends with a \land
...
% TODO: the first sentence comes out of nowhere
...
\begin{definition}\label{def:model}
  ...
  \end{enumerate}
\end{definition}
% TODO (about the second condition): inductive defin?
% M entails p(a) /\ p'(b) iff M entails p(a) and M entails p'(b)
% M entails \neg p(a) iff M \not\entails p(a)
% M entails \forall X. p(a) iff M entails p(d) \forall d \in D.

% TODO: make the relations more concrete
\begin{example}
...
% TODO: the definition of FCG is hard to miss (highlight its importance, maybe
% remind the reader later on in the paper)

% TODO: instead of FO d-DNNF computational graph, say 'first-order computational
% graph' (FOCG?)
...
% TODO (about the footnote): say: this shows that there is no loss of
% expressiveness/compilation completeness with our new algorithm. Would be nice
% if you could also show formally how it has improved on Guy, e.g., Prop.
% \exists formula \phi such that AlgoGuy(\phi) may fail but NewAlg(\phi) \ne
% \emptyset. Therefore, our algorithms is `more' complete than Guy's
...
\begin{proof}
  ...
  $\phi \equiv \phi'$.
\end{proof}
% TODO: Does this have analogue in Guy? If yes, explain difference. If no,
% emphasize this point. Same for Theorem 3.
...
\begin{algorithm}[t]
  ...
  $\variableNames \gets \{\, \Omega \mapsto \newVariableName{} \mid \Omega \in \mathcal{D} \,\}$\;\label{line:varnames}
  \If{$s$ is a direct successor of a $\Reff$ node}{
    $(\expression, \functions) \gets \visit{$s$, $\variableNames$}$\;\label{line:initialcall}
    \Return{\functions}\;\label{line:initialcalltwo}
  }
  $f \gets \newFunctionName{$s$}$\;
  $(\expression, \functions) \gets \visit{$s$, $\variableNames$}$\;
  \Return{$\langle f(\langle \variableNames{$\Omega$} \mid \Omega \in \mathcal{D} \rangle) = \expression \rangle \mdoubleplus \functions$}\;
  \Fn{\visit{node $v$, variable names \variableNames}}{
    \If{$v$ is \textbf{not} a direct successor of a $\Reff$ node}{
      \Return{\actuallyVisit{$v$, \variableNames}}\;
    }
    $f \gets \newFunctionName{$v$}$\;
    $\newVariableNames \gets \variableNames$\;
    \For{$\Omega \in \mathfrak{D}(v)$ s.t. $\newVariableNames(\Omega)$ is non-atomic}{\label{line:nonone}
      $\newVariableNames(\Omega) \gets \newVariableName{}$\;\label{line:nontwo}
    }
    $(\expression, \functions) \gets \actuallyVisit{$v$, \newVariableNames}$\;\label{line:actuallyvisit}
    $\functionCall \gets f(\langle \variableNames{$\Omega$} \mid \Omega \in \mathfrak{D}(v) \rangle)$\;
    $\functionSignature \gets f(\langle \newVariableNames{$\Omega$} \mid \Omega \in \mathfrak{D}(v) \rangle)$\;
    \Return{$(\functionCall, \langle \functionSignature = \expression \rangle \mdoubleplus \functions)$}\;
  }
  \Fn{\actuallyVisit{node $v$, variable names \variableNames}}{
    \Switch{label of $v$}{
      \lCase{$\GDR(v')$}{\Return{\visit{$v'$, \variableNames}}}
      \uCase{$\CR_{\Omega \mapsto \Omega'}(v')$}{
        \Return{\visit{$v'$, $\variableNames \cup \{\, \Omega' \mapsto (\variableNames(\Omega) - 1) \,\}$}}\;\label{line:cr}
      }
      \uCase{$\Reff_{\rho}(v')$}{
        $\args \gets \langle \variableNames\circ\rho(\Omega) \mid \Omega \in \mathfrak{D}(v') \rangle$\;\label{line:args}
...
\begin{figure}
  ...
  \begin{tikzcd}
    ...
    \visit{$\GDR$, $\Vone$} \ar[d] \ar[u, swap, shift right=0.7ex, "\left({f(m, n), \left\langle f(m, n) = \sum_{l=0}^{n}[l<2]f(m-1,n-l) \right\rangle}\right)"] & \\
    \actuallyVisit{$\GDR$, $\Vone$} \ar[d] \ar[u, swap, shift right=0.7ex, "\left({\sum_{l=0}^{n}[l<2]f(m-1,n-l), \langle\rangle}\right)"] & \\
    \visit/\actuallyVisit{$\bigvee$, $\Vone$} \ar[d] \ar[u, swap, shift right=0.7ex, "\left({\sum_{l=0}^{n}[l<2]f(m-1,n-l), \langle\rangle}\right)"] & \\
    \visit/\actuallyVisit{$\CR$, $\Vtwo$} \ar[d] \ar[u, swap, shift right=0.7ex, "\left({[l<2]f(m-1,n-l), \langle\rangle}\right)"] & \\
    \visit/\actuallyVisit{$\land$, $\Vthree$} \ar[d] \ar[dr] \ar[u, swap, shift right=0.7ex, "\left({[l<2]f(m-1,n-l), \langle\rangle}\right)"] & \\
    \visit/\actuallyVisit{$\bot$, $\Vthree$} \ar[u, swap, shift right=0.7ex, "\left({[l<2], \langle\rangle}\right)"] & \visit/\actuallyVisit{$\Reff$, $\Vthree$} \ar[ul, swap, shift right=0.7ex, "\left({f(m-1, n-l), \langle\rangle}\right)"]
  \end{tikzcd}
  \caption{Function calls and their return values when \cref{alg:interpretation}
    is run on the FCG from \cref{fig:examplefcg}. When \protect\visit calls
    \protect\actuallyVisit and returns its output unchanged, the two function
    calls are shortened as a single call to
    $\protect\visit/\protect\actuallyVisit$. TODO: this last sentence should be
    explained in text.}\label{fig:functioncalls}
\end{figure}
...
\begin{example}\label{example:interpretation}
  ...
  by \newVariableName. \Cref{line:varnames} initialises $\mathsf{V}$ to $\Vone$.
...
% determine)
% TODO: Must do one of these:
% 1. determine the complexity of the counting quantifiers algorithm when run on
% these instances
% 2. find instances that are not in C^2 and make the paper about them
% 3. run the counting quantifiers algorithm on these instances with increasing
% domain sizes, statistically estimating the degree of the polynomial and
% juxtaposing it with my complexity results shifted to overlap as much as
% possible
...
% TODO: remove the rows that say 'same as..', putting the explanation about them
% in text. And don't say 'same as', instead say that the formulas are in some
% sence equivalent (maybe in a stronger sense than the usual equivalence).

% TODO: emphasise that the reader should focus on the last two columns
% TODO: the last sentence of the caption could be confusing
% TODO: 'best known' sounds disappointing. Find a better alternative.

% TODO: I think I said this the last time, but alog with the complexities as a
