...
of the domain(s) \citep{jaeger2012liftability}. Over more than a decade, many
classes of instances were shown to be liftable
...
% \footnote{If some domain is not big enough to contain all of its constants,
...
with $x$.\footnote{If $(X, Y)$ is a two-variable constraint, substituting a
  constant $c$ for $X$ would result in $(c, Y)$, which would have to be
  rewritten as $(Y, c)$ to fit the definition of a constraint.}
