...
% grammarly the whole thing (supplementary material as well)
% make sure the result table (as well as other floating objects) are on the right pages
% make sure the abstract, introduction, and conclusion are all in line with each other
% maybe improve the indentation of equations
...
\begin{figure*}[t]
  ...
  \begin{subfigure}{\textwidth}
    ...
    \caption{\textsc{Crane} compiles $\phi$ into a graph $G_{\phi}$ and then
      ...
      If some of the functions are defined recursively, base cases need to be
...
\identifyRecursion is called to check whether a recursive call is feasible. If
it is, \identifyRecursion returns a (total) map
...
clauses of $\phi$ and $\psi$ that have the same hash codes. For every pair of
sufficiently similar clauses, \identifyRecursion calls itself on the remaining
clauses until the map $\rho\colon \Doms(\psi) \pfun \Doms(\phi)$ becomes total,
and all clauses are successfully coupled.
...
$(\expression, \functions)$. Here, \expression is the algebraic expression that
represent $v$, and \functions is a list of auxiliary functions that were created
while formulating \expression.
...
% and the rest follow the table of all that will be better [an experiment with
% non-partial endo injections?]
...
% However, we can do better and identify the exact worst-case asymptotic
...
with the exception of Beame et al.~\shortcite{DBLP:conf/pods/BeameBGS15}. The
...
% In particular, the complexity of computing total surjections is by
% Earnest~\shortcite{30049}, and all other such complexities are inferred from the
% formulas and programs on the on-line encyclopedia of integer
% sequences~\cite{oeis}.
