...
\paragraph{Evaluation.} $\WMC(\CR_{d \mapsto d'}(n), \size) = \WMC(n, \size \cup \{\, d' \mapsto \size(d) - 1 \,\})$.

TODO: add an explanation for why the way constraints, domain recursion, and constraint removal interact ensures that domain sizes will never be negative.
