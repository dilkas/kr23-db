...
Intuition: although one can return zero if the size of a domain is too big, there is no way to do the same if the domain size is too small. And this turns out to be crucial for computing factorials and constructing recursive subcircuits that compare the sizes of two domains.

\begin{theorem}
  There is no circuit $x$ such that
  \[
  \WMC(x; \size) =
  \begin{cases}
    0 & \text{if } \size(d) = 0 \\
  ...
  \end{cases}
  \]
  where $y$ is a circuit node of arbitrary type.
\end{theorem}
\begin{proof}
  Let us consider all possible root node types.
\end{proof}
