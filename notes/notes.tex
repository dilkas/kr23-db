...
\paragraph{TODO.}
\begin{itemize}
\item maybe add a (vertex) labelling function and its codomain (and use it in the algorithm)? But it's only used once...
...
\item need an example
\item vertex or node, $v$ or $n$?
\item what notation for lists (a.k.a. sequences) am I going to use? $\oplus$ for concatenation (of both two lists and an element to a list), $\in$ for (in-order) enumeration, $[]$ for an empty list, $[x]$ for a list with one element, $L$ for the length of the list.
\end{itemize}
...
\begin{algorithm}[t]
  ...
  \lIf{$L_0 = []$}{$\solutions \gets \solutions \cup \{\, G_0 \,\}$}
  \Else{
  ...
  \Fn{\applyGreedyRules{$\phi$, $C$}}{
    \ForEach{rule $r \in \Gamma$}{
      $S \gets r(\phi)$\;
      \If{$S \ne \emptyset$}{
        $(G, L) \gets \text{any element of } S$\; % TODO: explain why this is valid for greedy rules
